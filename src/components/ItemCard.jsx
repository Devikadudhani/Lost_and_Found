...
function LostFoundGallery() {
      ...
      {items.length === 0 ? (
        ...
        <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 scrollbar-hide">
          {items.map((item) => (
            <div
              key={item._id}
              className="min-w-[220px] sm:min-w-[250px] bg-white rounded-xl shadow hover:shadow-lg transition-transform hover:-translate-y-1"
            >
              <img
                src={item.imageUrl}
                alt={item.title}
  ...
  return (
    <div className="px-3 sm:px-4 md:px-8 py-6 sm:py-10 bg-gray-50 min-h-screen">
