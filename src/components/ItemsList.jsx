...
export default function ItemsList({ fixedType }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) alert("Please login to view.");
        setItems([]);
        setTotal(0);
        setLoading(false);
        return;
    ...
    <div className="max-w-6xl mx-auto px-4 py-4 sm:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <a
            ...
            className="px-4 py-2 bg-themeGreen text-white rounded text-sm sm:text-base"
          >
          ...
          <button
            ...
            className={`px-3 py-1 border rounded text-sm ${mineOnly ? "bg-gray-100" : ""}`}
          >
          ...
          {fixedType === "lost" && (
            <button
              ...
              className={`px-3 py-1 border rounded text-sm ${statusFilter === "found" ? "bg-gray-100" : ""}`}
            >
              {statusFilter === "found" ? "Found" : "Items Found"}
            </button>
          )}
          ...
          {fixedType === "found" && (
            <button
              ...
              className={`px-3 py-1 border rounded text-sm ${statusFilter === "handed" ? "bg-gray-100" : ""}`}
            >
              {statusFilter === "handed" ? "Handed" : "Items Handed"}
            </button>
          )}
      ...
      <div className="mb-4">
        <input
          ...
          placeholder={`Search ${fixedType ?? "items"}...`}
